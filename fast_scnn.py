...
def conv_block(inputs, conv_type, kernel, kernel_size, strides, padding='same', relu=True,
               reg=keras.regularizers.l2(0.00004)):
    if (conv_type == 'ds'):
        x = tf.keras.layers.SeparableConv2D(kernel, kernel_size, padding=padding, strides=strides)(inputs)
    else:
        x = tf.keras.layers.Conv2D(kernel, kernel_size, padding=padding, strides=strides, kernel_regularizer=reg,
...
def pyramid_pooling_block(input_tensor, bin_sizes, input_size):
    ...
    for bin_size in bin_sizes:
        ...
        x = tf.keras.layers.Conv2D(128, 3, 2, padding='same', kernel_regularizer=keras.regularizers.l2(0.00004),
                                   bias_regularizer=keras.regularizers.l2(0.00004))(x)
...
def model(num_classes=19, input_size=(256, 256, 3)):
    # Input Layer
